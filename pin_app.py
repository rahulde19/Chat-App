...
# pinned chat folder
def pin_chat(chat_name):
    pinned_chat_path = os.path.join(PIN_FOLDER, "pinned_chat.json")
    with open(pinned_chat_path, "w", encoding="utf-8") as f:
        json.dump({"pinned_chat": chat_name}, f, indent=4)
...
if selected_chat:
    if ("chat_name" not in st.session_state or
...
# pinning button is on the top right corner of the chat window, if clicked then pin the chat
